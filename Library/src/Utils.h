...
#pragma warning(disable : 4505) //Warning unreferenced local function
		...
		static ColorRGB Phong(float specularReflectance, float phongExp, const Vector3& lightDir, const Vector3& viewDir, const Vector3& normal)
		{
			...
			if (phongExp == 1) return specularReflectance * cos * ColorRGB{ 1,1,1 };
			if (phongExp == 2) return specularReflectance * cos * cos * ColorRGB{ 1,1,1 };
			return { specularReflectance * powf(cos,phongExp) * ColorRGB { 1,1,1 } };
		}
#pragma warning(pop)
	}
}